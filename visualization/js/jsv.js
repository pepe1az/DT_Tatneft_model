...
if (typeof JSV === 'undefined') {
  ...
  if (JSV.labels[d.name]) return;
  if (d3.event && d3.event.defaultPrevented) return;

  // 1) ОБЯЗАТЕЛЬНЫЙ ПЕРЕХОД: если получилось открыть зеркальные ветки — выходим
  if (JSV._isLeaf(d) && JSV.openMirrorAttribute(d)) {
  if (d3.event) d3.event.stopPropagation();
  return;
}

  // 2) Fallback: старое поведение по d.form/инфо
  ...
  if (!JSV.plain) {
    ...
    panel.panel('open');
        ...
        _foldYo: function (s) {
            return String(s == null ? '' : s).replace(/ё/g, 'е').replace(/Ё/g, 'Е');
            },
        _normLabel: function (s) {
            var t = JSV._foldYo(String(s || ""));          
            t = t.toLowerCase();                           // без регистра
        ...
        _branchOf: function(d){
  var cur = d;
  while (cur) {
    var raw = String(cur.name || '');
    var low = JSV._normName(raw);
    if (low === 'входные документы') return 'docs';
    if (low === 'базы данных') return 'db';
    if (/^бд\s+/i.test(raw)) return 'db';
    if (/\sформы$/i.test(raw)) return 'docs';
    cur = cur.parent;
  }
  return null;
},

_extractGroup: function(d, branch){
  var cur = d;
  while (cur) {
    var raw = String(cur.name || '');
    if (branch === 'db') {
      var m = raw.match(/^БД[.:]?\s+(.+)$/i) || raw.match(/^Бд[.:]?\s+(.+)$/i);
      if (m) return m[1].trim();
    } else if (branch === 'docs') {
      var m2 = raw.match(/^(.+?)\s+форм(?:ы)?$/i);
...
_hasAncestorNamed: function(d, nameLike){
  var want = JSV._normName(nameLike);
  var cur = d;
  while (cur) {
    if (JSV._normName(cur.name) === want) return true;
    cur = cur.parent;
  }
  return false;
},
_hasAncestorNamedAny: function(d, names){
  if (!names || !names.length) return false;
  var set = Object.create(null);
  names.forEach(function(n){ var k = JSV._normName(n); if (k) set[k]=1; });
  var cur = d;
  while (cur) {
    var k = JSV._normName(cur.name);
...
_groupTargetCandidates: function(core, toBranch){
  var c = String(core || '').trim();
  if (!c) return [];
  if (toBranch === 'docs') {
    var base = [c + ' формы', c + ' форм'];
    var key = c.toLowerCase();
    if (JSV._groupMapOverrides && JSV._groupMapOverrides[key]) {
      JSV._groupMapOverrides[key].forEach(function(x){
        if (base.indexOf(x) === -1) base.push(x);
      });
    }
    return base;
  } else {
    return ['БД ' + c, 'Бд ' + c, 'БД: ' + c, 'БД. ' + c];
  }
},
_buildLabelIndex: function(){
  var index = Object.create(null);
  function put(lbl, node){
    var key = JSV._normLabel(lbl);
    if (!key) return;
    (index[key] || (index[key] = [])).push(node);
  }
  JSV.visit(JSV.treeData, function(n){
    if (!n) return;
    // НОВОЕ: индексируем ТОЛЬКО листья
    if (!JSV._isLeaf(n)) return;

    if (n.plainName) put(n.plainName, n);
    if (n.name && n.name !== n.plainName) put(n.name, n);

    if (n.translation && typeof n.translation === 'object') {
      Object.keys(n.translation).forEach(function(lang){
        var vals = n.translation[lang];
        if (Array.isArray(vals)) vals.forEach(function(v){ put(v, n); });
      });
    }
  }, function(n){ return (n && (n.children || n._children)) || null; });
...
_collectSynonymKeys: function(base){
  var k = JSV._normLabel(base);
  if (!k) return [];
  var out = [k];
  var adj = JSV._synAdj || {};
  if (adj[k]) out = out.concat(Object.keys(adj[k]));
  var seen = Object.create(null), uniq = [];
  for (var i=0;i<out.length;i++){ var x=out[i]; if (!seen[x]){ seen[x]=1; uniq.push(x);} }
  return uniq;
},

openMirrorAttribute: function(d){
  try {
    if (!JSV._isLeaf(d)) return false;

    var branch = JSV._branchOf(d);
    if (!branch) return false;
    var toBranch = (branch === 'db') ? 'docs' : 'db';

    var groupCore  = JSV._extractGroup(d, branch);
    var targetNames = groupCore ? JSV._groupTargetCandidates(groupCore, toBranch) : [];

    var baseLabel = d.plainName || d.name || '';
    var keys = JSV._collectSynonymKeys(baseLabel);
    if (!keys.length) keys = [JSV._normLabel(baseLabel)];

    var index = JSV._buildLabelIndex(); // индекс только по листьям
    var candidates = [];

    function pushIfCandidate(arr, filterByGroup){
      for (var i=0;i<arr.length;i++){
        var n = arr[i];
        if (!JSV._isLeaf(n)) continue;                 // подстраховка
        if (JSV._branchOf(n) !== toBranch) continue;
        if (filterByGroup && targetNames.length && !JSV._hasAncestorNamedAny(n, targetNames)) continue;
    ...
    for (var ki=0; ki<keys.length; ki++){
      pushIfCandidate(index[keys[ki]] || [], /*filterByGroup*/ true);
    }

    // 2) подстроки/точные
    if (!candidates.length){
      var labelKey = JSV._normLabel(baseLabel), karr = Object.keys(index);
      for (var kk=0; kk<karr.length; kk++){
        var k2 = karr[kk];
        if (k2 === labelKey) continue;
        if (k2.indexOf(labelKey) === -1 && labelKey.indexOf(k2) === -1) continue;
        pushIfCandidate(index[k2] || [], /*filterByGroup*/ true);
      }
    }

    // 3) если знаем группу — ещё одна попытка без фильтра по группе
    if (!candidates.length && targetNames.length) {
      for (var ki2=0; ki2<keys.length; ki2++){
        pushIfCandidate(index[keys[ki2]] || [], /*filterByGroup*/ false);
      }
    }
    if (!candidates.length){
  var labelKey2 = JSV._normLabel(baseLabel), allKeys = Object.keys(index);
  for (var s = 0; s < allKeys.length; s++){
    var k2 = allKeys[s];
    if (k2 === labelKey2) continue;
    if (k2.indexOf(labelKey2) === -1 && labelKey2.indexOf(k2) === -1) continue;
    pushIfCandidate(index[k2] || [], /*filterByGroup*/ false);
  }
}
    if (!candidates.length) return false; // НЕТ фоллбэка к веткам/группам

    // раскрываем найденные листья
    var opened = [];
    for (var j=0;j<candidates.length;j++){
      var path = JSV.getNodePath(candidates[j]);
      var node = JSV._expandIndexPathNoCenter(path);
      if (node) { JSV._expandAll(node); opened.push(node); }
    }
    if (!opened.length) return false;

    JSV.update(JSV.treeData);
    var centerTarget = opened[opened.length - 1];
    if (centerTarget) JSV.centerNode(centerTarget);
    opened.forEach(function(n, idx){
      setTimeout(function(){ JSV.flashNode(n); }, idx * 160);
    });

    return true;
  } catch (e) {
    console.warn('openMirrorAttribute error:', e);
    return false;
  }
},

nodeCircleClick: function(d){
  // 1) Сначала пробуем обязательный переход (и для листьев, и для веток)
  if (JSV._isLeaf(d) && JSV.openMirrorAttribute(d)) {
  if (d3.event) d3.event.stopPropagation();
  return;
}
  // 2) Если перехода нет — стандартное сворачивание/разворачивание
  JSV.click(d);
},
  ...
  // индекс: НОРМАЛИЗОВАННАЯ_МЕТКА -> [узлы-листья с этой меткой]
  var index = Object.create(null);
  ...
  // проверяем реальное наличие пары в другой ветке
  Object.keys(adj).forEach(function (lbl) {
    var neigh = adj[lbl];
    var nodesA = index[lbl];
    if (!neigh || !nodesA || !nodesA.length) return;

    Object.keys(neigh).forEach(function (nb) {
      var nodesB = index[nb];
      if (!nodesB || !nodesB.length) return;

      var a_db   = nodesA.filter(function(n){ return JSV._branchOf(n) === 'db'; });
      var a_docs = nodesA.filter(function(n){ return JSV._branchOf(n) === 'docs'; });
      var b_db   = nodesB.filter(function(n){ return JSV._branchOf(n) === 'db'; });
      var b_docs = nodesB.filter(function(n){ return JSV._branchOf(n) === 'docs'; });

      var hasDbDocs  = (a_db.length && b_docs.length) || (a_docs.length && b_db.length);
      if (!hasDbDocs) return;

      // помечаем только те узлы, для которых реально есть «партнёры» в другой ветке
      [].concat(a_db, a_docs, b_db, b_docs).forEach(function(n){ n._synLinked = true; });
    });
  });
})();
    (function resolveDirectLinks () {
  ...
  // собираем ТОЛЬКО листья по их именам/лейблам
  JSV.visit(JSV.treeData, function(d){
  if (!d._isLeafNode) return;
  if (d.plainName) put(d.plainName, d);
  if (d.name && d.name !== d.plainName) put(d.name, d);
  ...
  // (i) точные совпадения ключа между ветками
  Object.keys(index).forEach(function (k) {
    var arr = index[k] || [];
    var hasDb = arr.some(function (n) { return JSV._branchOf(n) === 'db'; });
    var hasDocs = arr.some(function (n) { return JSV._branchOf(n) === 'docs'; });
    if (hasDb && hasDocs) {
      arr.forEach(function (n) { n._dirLinked = true; });
    }
  });

  // (ii) включения по подстроке (A ⊂ B или B ⊂ A), тоже только если обе ветки представлены
  ...
  for (var i = 0; i < keys.length; i++) {
    for (var j = i + 1; j < keys.length; j++) {
      ...
      var union = (index[a] || []).concat(index[b] || []);
      var hasDb   = union.some(function (n) { return JSV._branchOf(n) === 'db';   });
      var hasDocs = union.some(function (n) { return JSV._branchOf(n) === 'docs'; });
      if (hasDb && hasDocs) {
        union.forEach(function (n) { n._dirLinked = true; });
      }
    }
  }
            ...
            function leafFill(d) {
  if (d && d._isLeafNode) return (d._synLinked || d._dirLinked) ? '#059669' : '#dc2626';
  return null;
}
            var maxW = {};
            var maxDepth = 0;
